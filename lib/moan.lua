...
-------------------------------------------------
Moan = {
  indicatorCharacter = "<<< PRESS SPACE BAR >>>",
  optionCharacter = "> ",
  indicatorDelay = 25,
  selectButton = "space",
  typeSpeed = 0.01,
...
function Moan.draw()
  ...
  if Moan.showingMessage then
    ...
    local optionsY = textY+Moan.font:getHeight(printedText)-(padding/1.25)
    local optionsSpace = fontHeight/1.15  --CHANGE THE SPACE BETWEEN THE OPTIONS
    ...
    if Moan.showIndicator then
      if not (Moan.UI.imagePos == "right" and type(Moan.currentImage) == "userdata") then
        love.graphics.print(Moan.indicatorCharacter, boxX+boxW-(27*padding), boxY+boxH-(padding/2)-fontHeight)
      end
    end
  end
